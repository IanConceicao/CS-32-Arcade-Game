...
bool Zombie::getAttacked(int dir)  // return true if dies
{
    ...
    return true;
    //return true if attacked before
...
{
    m_age++;
    int colToAttack = m_col;
    int rowToAttack = m_row;
    ...
    if(m_arena->numZombiesAt(rowToAttack, colToAttack) >= 1){
        m_arena->attackZombieAt(rowToAttack, colToAttack, dir);
        return;
    }
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++){
            if(numZombiesAt(r + 1, c + 1) == 1)
                grid[r][c] = 'Z';
            else if(numZombiesAt(r + 1, c + 1) >= 2 && numZombiesAt(r + 1, c + 1) <= 9)
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    if(numZombiesAt(r, c) >= 1){
        //cycle through zombies, find one at the correct position
        for(int i = 0; i < m_nZombies; i++){
            if(m_zombies[i]->row() == r && m_zombies[i]->col() == c){
                if(m_zombies[i]->getAttacked(dir)){ //attack that zombie, see if it dies
                    delete m_zombies[i];
